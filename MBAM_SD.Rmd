...
```{r}
type_promo <- read_excel("Dictionnaire_MBAM.xlsx",sheet = "Type de promotion",col_names = TRUE)
```
```{r}

vars_promo <- c("code.promo", "1_2ans", "Type_promo", "Qte_promo1", "Qte_promo2","Detail_promo")

testing2 <- testing2 %>%
  mutate(code.promo=case_when(is.na(as.character(code.promo))==TRUE~"no_promo", TRUE~as.character(code.promo))) %>%
  merge(type_promo[vars_promo], by="code.promo") %>% distinct()
...
```{r}
#Change Quantity promo

testing3$Type_promo <- ifelse(testing3$campagnes_PromoR.ouverture20.==TRUE, "Rabais_pct", testing3$Type_promo)
testing3$Qte_promo1 <- ifelse(testing3$campagnes_PromoR.ouverture20.==TRUE, 0.2, testing3$Qte_promo1)
...
```{r}
write.csv(testing3, "FinalData.csv", row.names = FALSE)
```
