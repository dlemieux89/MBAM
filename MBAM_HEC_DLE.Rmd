...
```{r}
#Load data
complete_df <- read.csv("C:\\Users\\David\\OneDrive\\1_Ecole\\HEC\\Msc\\HEC DATA\\MBAM/MBAM_data/MBAM_complet.csv")
conversion <- read.csv("C:\\Users\\David\\OneDrive\\1_Ecole\\HEC\\Msc\\HEC DATA\\MBAM/MBAM_data/MBAM_convertis.csv")
...
campagnes_date= complete_df %>% select(date,campagnes) %>%  group_by(date) %>% summarise(n_campagne_day= length(unique(campagnes))) 
...
complete_df_adj <- complete_df_adj %>% ungroup()  %>% distinct(.keep_all=TRUE) 
...
#testing <- read.csv("Curated_complete.csv")
...
testing2 <- testing %>%
              merge(
                complete_df[c("num_dossier_mbam_coded","first_transaction")] %>% distinct(), by="num_dossier_mbam_coded")
```
...
conversion_clean <- conversion_clean %>% filter(!(conversion_clean$n == 2 & conversion_clean$datecheck == FALSE))
...
# Remove code promo and other that we do not need
testing2 <- testing2 %>% select(-c(langue_.N.A,code.promo,first_transaction.y))
...
testing2 <- testing2 %>%
  arrange(num_dossier_mbam_coded, date) %>%
  group_by(num_dossier_mbam_coded) %>%
  mutate(emailnum = 1:n())
```
...
```{r }
yo <- testing2 %>%
        select(c(num_dossier_mbam_coded,date,open,last)) %>%
        arrange(date)%>%
        group_by(num_dossier_mbam_coded) %>%
...
yo<-yo %>%
  ...
  mutate(one1=1)%>%
  mutate(nb_courriel_until_converted= cumsum_reset(as.logical(1-last),one1))
...
```{r}
#Merge yo to testing2
testing3 <- testing2 %>%
                merge(yo %>% select(-c("last","one1","open")), all.x=TRUE, by = c("num_dossier_mbam_coded", "date")) %>% distinct()

campagnes_date= complete_df %>% select(date,campagnes) %>% distinct()
Dict <- read_excel("Dictionnaire_MBAM.xlsx",sheet= "Type de promotion")
Dict <- Dict %>% select(-c("code.promo",`1_2ans`,Qte_promo1,Qte_promo2,Detail_promo))%>% distinct()
campagnes_date_promo <- campagnes_date %>% merge(Dict,by="campagnes") 

campagnes_date_promo$date <- as.Date(campagnes_date_promo$date)
```

```{r}

#Add day of the week
testing3$day <- weekdays(testing3$date)
testing3$n <- NULL

data_final <- testing3 %>% merge(campagnes_date_promo,by="date", all.x=TRUE) 
```

```{r}
write.csv(data_final, "FinalData.csv", row.names = FALSE)
```
...
testing2 %>%
  filter(last == 1) %>%
  summarise(avgEmail = mean(emailnum))
